...
\begin{abstract}
This paper proposes a bit-flipping decoding algo- rithm suitable for decoding 
multiple correlated sources in a noisy environment.  Correlation models commonly 
...
number of sources (up to 100) are considered. Additionally, the correlation 
values of the models are generated randomly. The correlated sources are 
...
performance. Although the proposed algorithm do not make any estimation of 
log-likelihood values, it can be used in conjunction with a fusion rule for 
...
\caption{Bit error rate for a system with $3$ sources using Independent Decoding ("Independent") 
